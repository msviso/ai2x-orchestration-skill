...
??README 供人類????使??????發??、審??者、Beta 使用??????AI / agent ??????????????SKILL.md ??????本 repository 定義??是行為契????使????範??????實????執行細節??
## Version & Release Status
??????本??v0.1.0，屬??Private / Closed Beta??v0.x ??段不??證??後相容??行為????範????能調整??v1.0 ????行為契??穩??，schema ??canonical examples 視為對????諾??
## Private Beta Notice
??skill ?????????測試??段??API??schema ??canonical examples ??能??新??調??。本??件不????任??SLA ??????????用??承諾??
## Copyright & Ownership
c Microsense Vision Co., Ltd. ??repository ??於定義 AI skill ????????使用契??，實作細節??????除以??低耦????誤??風????
...
~~~json
{
  ...
  "data": {
    "title": "????",
    "items": [
      { "title": "????", "description": "請查??????????, "meta": "??新???? 10:00" }
    ]
  }
}
~~~
